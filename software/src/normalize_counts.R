...
process_RNASeq <- function(raw_counts_path, output_folder, species) {
  ...
  rownames(normalized_counts) <- sub("\\.\\d+$", "", rownames(normalized_counts))
  
  # Define your Ensembl IDs from the count matrix
  ...
  # Reorder columns
  normalized_counts_long <- normalized_counts_long[, c("Sample", "Geneid", "SYMBOL", "NormCounts")]
  
  # Write normalized counts to CSV
  normalized_counts_path <- file.path(output_folder, "normalized_counts.csv")
  write.csv(normalized_counts_long, file = normalized_counts_path, row.names = FALSE)
